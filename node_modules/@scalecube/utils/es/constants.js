...
export const getServiceNameInvalid = (serviceName) => `MS0023 - Invalid format, serviceName must be not empty string but received type ${typeof serviceName}`;
export const getIncorrectMethodValueError = (qualifier) => `Method value for ${qualifier} definition should be non empty object`;
export const getAsynModelNotProvidedError = (qualifier) => `Async model is not provided in service definition for ${qualifier}`;
export const getInvalidAsyncModelError = (qualifier) => `Invalid async model in service definition for ${qualifier}`;
